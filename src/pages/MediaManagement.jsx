...
import { Image as ImageIcon, Trash2, Upload, Layout, Loader2, Check, X } from 'lucide-react';
...
const GalleryManagement = () => {
   ...
   const handleFileUpload = async (e) => {
      ...
      try {
         ...
         const saveRes = await fetch(`${config.API_BASE_URL}/api/images/${type}`, {
            method: 'POST',
   ...
   return (
      <div className="container-fluid p-0 reveal in">
         <header className="mb-5">
            <h1 className="h3 fw-bold mb-2 text-white">Media Management</h1>
            <p className="text-muted-custom small">Browse and host your VTC visuals directly on Cloudinary.</p>
         </header>

         {loading ? (
            <div className="text-center py-5">
               <Loader2 size={48} className="animate-spin text-accent opacity-50 mb-3 mx-auto" />
               <div className="h5 text-white fw-bold mb-1">Synchronizing Storage Hub...</div>
            ...
            <>
               <div className="data-table p-5 mb-5 border-0 position-relative overflow-hidden">
                  ...
                  <div className="row align-items-center position-relative">
                     <div className="col-md-5">
                        <h4 className="fw-bold text-white mb-1">Upload New Media</h4>
                        <p className="small text-muted-custom mb-4">Select the destination and browse your local files.</p>
                        <select className="form-control bg-dark text-white border-white border-opacity-10 py-3 rounded-4 mb-3"
                           value={type} onChange={e => setType(e.target.value)}>
                           <option value="gallery">VTC Gallery Collection</option>
                           <option value="headers">Homepage Hero Banner</option>
                        </select>
                     </div>
                     <div className="col-md-7">
                        <div className="upload-zone p-5 rounded-4 border-2 border-dashed d-flex flex-column align-items-center justify-content-center gap-3 transition-all"
                           ...
                           {uploading ? (
                              ...
                              <label className="w-100 h-100 cursor-pointer text-center py-5 m-0 position-relative">
                                 ...
                                 <h5 className="text-white fw-bold">Click to Browse Local Files</h5>
               ...
               <div className="row g-4">
                  <div className="col-md-6 col-lg-5">
                     <div className="data-table p-5 h-100 border-0">
                        <h5 className="mb-5 d-flex align-items-center gap-2 text-white fw-bold">
                           <Layout size={24} className="text-accent" /> Homepage Banners
                        </h5>
                        <div className="row g-4 overflow-auto pe-2" style={{ maxHeight: 600 }}>
                           {headers.map(img => (
                              <div key={img.id} className="col-12 position-relative group animate-reveal">
                                 <div className="overflow-hidden rounded-4 border border-white border-opacity-10 shadow-sm position-relative">
                                    <img src={img.image_url} className="w-100 transition-transform duration-500 hover:scale-110" style={{ height: 160, objectFit: 'cover' }} alt="" />
                                    <button
                                       onClick={() => handleDelete('headers', img.id)}
                           ...
                           {headers.length === 0 && <div className="text-muted-custom small py-5 text-center w-100 h6 opacity-50 fw-normal">No home banners.</div>}
                        </div>
                     </div>
                  </div>
                  ...
                  <div className="col-md-6 col-lg-7">
                     <div className="data-table p-5 h-100 border-0">
                        <h5 className="mb-5 d-flex align-items-center gap-2 text-white fw-bold">
                           <ImageIcon size={24} className="text-accent" /> Gallery Collection
                        </h5>
                        <div className="row g-3 overflow-auto pe-2" style={{ maxHeight: 600 }}>
                           {gallery.map(img => (
                              <div key={img.id} className="col-md-6 col-lg-4 position-relative group animate-reveal">
                                 <div className="overflow-hidden rounded-4 border border-white border-opacity-10 shadow-sm position-relative">
                                    <img src={img.image_url} className="w-100 transition-transform duration-500 hover:scale-110" style={{ height: 140, objectFit: 'cover' }} alt="" />
                                    <button
                                       onClick={() => handleDelete('gallery', img.id)}
                                       className="btn btn-danger btn-sm rounded-circle p-2 position-absolute top-0 end-0 m-2 shadow-lg opacity-0 group-hover:opacity-100 transition-all border-0"
                                       style={{ background: 'rgba(220, 53, 69, 0.9)', backdropFilter: 'blur(5px)' }}
                                    >
                                       <Trash2 size={14} />
                                    </button>
                           ...
                           {gallery.length === 0 && <div className="text-muted-custom small py-5 text-center w-100 h6 opacity-50 fw-normal">Gallery is empty.</div>}
                        </div>
                     </div>
                  </div>
               </div>
               ...
               <div className="data-table p-0 border-0 shadow-2xl reveal zoom w-100 overflow-hidden" style={{ maxWidth: 450 }}>
                  <div className="p-5 text-center">
                     <div className={`rounded-circle mx-auto d-flex align-items-center justify-content-center mb-4 border-2 border ${statusModal.type === 'error' ? 'text-danger border-danger' : 'text-accent border-accent'}`}
                        ...
                        {statusModal.type === 'error' ? <X size={32} /> : statusModal.type === 'confirm' ? <Layout size={32} /> : <Check size={32} />}
                     </div>
